...
echo ##SBATCH --partition=prepost           # access to octo-gpus machines
...
mkdir -p $OUTPUT_LOCATION/features_syn_small/syntactic/{'dev','test'}

for item in ${KIND[*]}
do
  datafiles="${ZEROSPEECH_DATASET}/syntactic/${item}"
  output="${OUTPUT_LOCATION}/features_syn_small/syntactic/${item}"
...
for item in ${KIND[*]}
do
  quantized="$OUTPUT_LOCATION/features_syn_small/syntactic/${item}/quantized_outputs.txt"
  output="$OUTPUT_LOCATION/features_syn_small/syntactic/$item.txt"
...
FEATURES_LOCATION="${OUTPUT_LOCATION}/features_syn_small"
...
# just a trick even though not using zerospeech
if [[ ${KIND[*]} =~ (^|[[:space:]])"test"($|[[:space:]]) ]] ; then
  export ZEROSPEECH2021_TEST_GOLD=$ZEROSPEECH_DATASET
fi;

zerospeech2021-evaluate --no-phonetic --no-lexical --no-semantic --njobs $NB_JOBS -o "$OUTPUT_LOCATION/scores/syntax_small" $ZEROSPEECH_DATASET $FEATURES_LOCATION

# copy the score on $SCRATCH
mkdir -p $CHECKPOINT_LOCATION/$MODEL/scores/syntax_small
cp -r $OUTPUT_LOCATION/scores/syntax_small $CHECKPOINT_LOCATION/$MODEL/scores
